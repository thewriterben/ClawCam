...
The stdio server supports `initialize`, `tools/list`, `tools/call`, and `ping` in both `legacy-2024` and `stateless-2026` protocol modes, exposing all 57 tools (46 auto-approved / 11 gated). The Oh-Ben-Claw adapter (`brain/oh-ben-claw-adapter/`) consumes it; see `clawcam_gateway/mcp_server/README.md`.

Import firmware SD-card bundle artifacts generated by the ESP32-S3-EYE bench firmware:
...
A firmware bundle should contain `events/*.json`, `media/*`, and optionally `metadata/*.json`. The importer validates each event, auto-registers an unknown firmware node as a conservative `node` device by default, copies media into `--media-dir`, and stores the event in the same gateway database used by the dashboard and agent tools. Use `--reference-media` to keep media in place rather than copying it.
