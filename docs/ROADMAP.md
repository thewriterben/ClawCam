...
| Device positions (mappable nodes) | `latitude`/`longitude` on `devices` (the "node positions within a site" contract); populated on `upsert_device` (COALESCE-kept on re-upsert), backfilled; `set_device_position` (place a planned node), `devices_with_position`, `devices_in_site` (point-in-polygon). | ✅ **Done** — 7 tests in `tests/gateway/test_device_geo.py`. |
...
| Geo REST/MCP surface     | Expose site + device-position queries + CRUD. Reads via MCP (`list_sites`, `get_site_events`, `list_device_positions`, `get_site_devices`, SSOT-wired auto-approved) **and** REST; site CRUD, deployment→site link, and device positioning via REST. | ✅ **Done** — `GET/POST /api/v1/sites`, `GET /api/v1/sites/{id}`, `GET /api/v1/sites/{id}/events`, `GET /api/v1/sites/{id}/devices`, `GET /api/v1/devices/positions`, `PUT /api/v1/devices/{id}/position`, `PUT /api/v1/deployments/{id}/site`; catalog +4 (→53); `tests/gateway/test_geo_surface.py`. |
