...
| Firmware | **Scaffold** | ESP-IDF component layout exists, WildCAM migration interfaces are defined, ESP32-S3-EYE is selected as the first unverified camera target, and the camera component now has a gated `esp32-camera` hardware path plus ESP32-S3-EYE build defaults, boot-time capture smoke test, and gated SD/FATFS media/metadata persistence. Physical hardware validation is still required. |
