...
| Gateway service | **Scaffold** | Python package layout, API, SQLite persistence, schema validation, sample import CLI, firmware SD-card bundle importer, first MCP-style tool functions, HTTP tool dispatch, dashboard JSON, and a minimal local HTML dashboard exist. Runtime is still local-development grade. |
...
| Firmware | **Scaffold** | ESP-IDF component layout exists, WildCAM migration interfaces are defined, ESP32-S3-EYE is selected as the first unverified camera target, and the camera component now has a gated `esp32-camera` hardware path plus ESP32-S3-EYE build defaults, boot-time capture smoke test, gated SD/FATFS media/metadata persistence, gateway-compatible event artifact generation, and optional HTTP upload scaffolding for device/event posts. Physical hardware and Wi-Fi validation are still required. |
